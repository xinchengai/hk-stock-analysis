...
- 对每只港股都检查流动性风险，尤其是成交额、换手率、价差和自由流通情况。
- 对符合条件的公司才启用 AH 溢价、南向资金、卖空和行业专项模块，不硬套模板。
...
```text
分析中国平安这支港股，补充 AH 溢价、南向资金和卖空数据
```

分析互联网平台公司：

```text
分析腾讯控股，重点看 FCF、回购、用户增长和估值口径
```
...
references/hk-market-modules.md
references/industry-frameworks.md
